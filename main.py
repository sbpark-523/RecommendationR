...
def command_line_flags(argv):
    script = argv[0]

    for act_idx in range(1, len(argv), 2):
        if argv[act_idx] == '--window':
            myCfg.window_length = int(argv[act_idx + 1])
            myCfg.total_windows = myCfg.window_calc()
        elif argv[act_idx] == '--method':
            myCfg.method = argv[act_idx + 1]
        elif argv[act_idx] == '--input':
            myCfg.data_route = argv[act_idx + 1]
